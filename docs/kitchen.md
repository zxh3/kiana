...
kitchen is a thin console over compute the user already owns. You bring a Modal
token; kitchen creates sandboxes in *your* Modal workspace and gives you three
...
"kianax" and later renamed to **kitchen** (pronounced "cook"; future home: kitchen.dev on Cloudflare Workers)). This doc transcribes that design
...
- State the machine truth: "Runs in your Modal workspace." Never claim work
  Kitchen does not do. "Sandboxes are billed by Modal, not by Kitchen."
...
| Layer | Choice |
|---|---|
| App | SvelteKit (Svelte 5, runes), client-rendered (`ssr = false`) — existing `apps/kitchen` |
...
- **Sandbox memory** — `kitchen-sandboxes:<workspace>` in localStorage: spec +
...
- **Credentials** — `{tokenId, tokenSecret, environment, workspace}` in the
  ...
  exist.
- **Sandbox spec** — stored as Modal tags at create time (`kitchen=1`,
  `kitchen-name/-image/-cpu/-memory/-gpu/-gpu-count/-volumes/-created`), so
...
- **Pane auth secret** — derived, never stored:
  `HMAC-SHA256(tokenSecret, "kitchen-pane:" + name)`, computed per request and
  injected as boot env at create.
- **Persistent state** — the shared `kitchen-state` volume, subPath keyed by
...
/api/sandboxes        GET  list (kitchen-tagged, running)  ·  POST create
...
- `listSandboxes` — `sandboxes.list({appId, tags: {kitchen: "1"}})` +
...
persistence, writes the Caddyfile from `KITCHEN_SECRET`, and starts the
...
back/forward would need same-origin proxying through the kitchen server.
...
**Auth.** Pane iframes load `https://<tunnel>/kitchen-auth?token=<secret>`;
...
terminate; type kitchen for details"); and a `kitchen` command baked into every
...
`/kitchen-state/tools/…`, so `rustup` and `npm i -g` survive restarts without
...
**Persistence.** One shared Modal volume (`kitchen-state`) serves the whole
workspace; each sandbox mounts the subPath `sandboxes/<name>` at
/kitchen-state — keyed by name, so terminating a sandbox and creating another
...
and the volumes popover lists `kitchen-state → /workspace` as a built-in row
...
`/kitchen-state/agents/…`), pi via a `~/.pi` symlink, plus bash history via
...
`HERDR_SOCKET_PATH=/tmp/herdr.sock`. `/workspace` and `/kitchen-state` are
...
### 3.7 Deployment — kitchen runs on Modal

`apps/kitchen/deploy.py` deploys the adapter-node build as a Modal Server
...
Live at `https://<workspace>--kitchen-kitchenserver.us-east.modal.direct` (deploy
into a specific Modal environment with `modal deploy --env <name>`; e.g. the
modal-labs/xiaohua-dev instance lives at
`modal-labs-xiaohua-dev--kitchen-kitchenserver.us-east.modal.direct`) —
scale-to-zero, `unauthenticated=True` (the app has its own credential model;
API calls without Modal credentials get 401). Point kitchen.dev at it via the
...
- Stopped / failed / archived sandboxes — Modal lists running sandboxes only.
  ...
  need a store (a `kitchen-meta` Modal volume is the natural one, not a DB).
