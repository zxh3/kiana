/**
 * Restore points: qook's whole persistence model.
 *
 ...
 * the same Modal credentials. Nothing about persistence is stored by qook
...
/**
 * Restore-point routes talk to Modal directly rather than through a wrapper
 * per operation; this keeps the credential plumbing in one place. Callers own
 * the client's lifetime and must `close()` it.
 */
export function clientForCredentials(creds: ModalCredentials): ModalClient {
...
/** Inverse of `buildTag`. Returns null for tags qook did not write. */
export function parseTag(
  ...
  const [retention, runtime, , label = ""] = version.split(".");
  ...
  return {
    ...
    expiresAt: autoDays
      ? new Date(
          createdAt.getTime() + Number(autoDays[1]) * DAY_MS,
        ).toISOString()
      : null,
    imageId,
  };
...
export async function listRestorePoints(
  ...
  let pageToken = "";
  do {
    const page = await client.cpClient.imageListTags({
      environmentName: "",
...
export async function newestRestorePoint(
  ...
  return (await listRestorePoints(client, sandbox))[0] ?? null;
}
...
export async function saveRestorePoint(
  ...
  if (!point) throw new Error(`qook built an unparseable tag: ${tag}`);
  return point;
}
 ...
 */
export async function pinRestorePoint(
  client: ModalClient,
  point: RestorePoint,
  label: string,
  now = new Date(),
): Promise<RestorePoint> {
  const app = await client.apps.fromName("qook", { createIfMissing: true });
  const source = await client.images.fromId(point.imageId);
  const pinned = await source.dockerfileCommands(["RUN true"]).build(app);

  const tag = buildTag(
    point.sandbox,
    null,
    slugifyLabel(label) || `pinned-${point.createdAt.slice(0, 10)}`,
    now,
  );
  await pinned.publish(tag);
  const result = parseTag(tag, pinned.imageId, now);
  if (!result) throw new Error(`qook built an unparseable tag: ${tag}`);
  return result;
}
...
export async function imageForPoint(
  ...
  const [name, version] = tag.split(":");
  return client.images.fromName(`${name}:${version}`);
}
...
export async function deleteRestorePoint(
  ...
  await client.images.delete(point.imageId);
}
...
export async function deleteAllRestorePoints(
  ...
  const points = await listRestorePoints(client, sandbox);
  const results = await Promise.allSettled(
    points.map((p) => client.images.delete(p.imageId)),
  );
