...
export const GET: RequestHandler = async ({ request, url }) => {
  ...
  const client = clientForCredentials(creds);
  try {
    return json({ points: await listRestorePoints(client, sandbox) });
  } catch (e) {
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    return json({ point: await pinRestorePoint(client, point, label) });
  } catch (e) {
    return json({ error: modalErrorMessage(e) }, { status: 502 });
  } finally {
    client.close();
...
export const DELETE: RequestHandler = async ({ request, url }) => {
  ...
  try {
    if (sandbox) {
      return json({ deleted: await deleteAllRestorePoints(client, sandbox) });
    }
    if (!tag) {
      return json({ error: "Missing ?tag= or ?sandbox=." }, { status: 400 });
    }
    const name = tag.split(":")[0].replace(/^qook-snap-/, "");
    const point = (await listRestorePoints(client, name)).find(
      (p) => p.tag === tag,
    );
    if (point) await deleteRestorePoint(client, point);
